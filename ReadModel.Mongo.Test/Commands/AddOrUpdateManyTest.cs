...
namespace Spritely.ReadModel.Mongo.Test
{
    ...
    [TestFixture]
    public class AddOrUpdateManyTest
    {
        ...
        public void Setup()
        {
            storageModels = StorageModel.CreateMany(nameof(AddOrUpdateManyTest), count: 5);
            testModels = TestModel.CreateMany(nameof(AddOrUpdateManyTest), count: 5);
        }
        ...
        public void Adds_when_model_does_not_exist_in_database()
        {
            ...
            var addOrUpdateTask = database.AddOrUpdateManyTestModels(models);
        ...
        public void Adds_when_model_does_not_exist_in_database_with_custom_metadata()
        {
            ...
            var addOrUpdateTask = database.AddOrUpdateManyStorageModels(models);
            addOrUpdateTask.Wait();
        ...
        {
            Assert.That(
                () => Commands.AddOrUpdateManyAsync<TestReadModelDatabase, Guid, TestModel>(null),
        ...
        {
            Assert.That(
                () => Commands.AddOrUpdateManyAsync<TestReadModelDatabase, Guid, TestModel, TestMetadata>(null),
                Throws.TypeOf<ArgumentNullException>());
        }
        ...
        {
            Assert.That(
                () => Task.Run(() => database.AddOrUpdateManyTestModels(null)).Wait(),
        ...
        {
            Assert.That(
                () => Task.Run(() => database.AddOrUpdateManyStorageModels(null)).Wait(),
                Throws.TypeOf<AggregateException>().With.InnerException.TypeOf<ArgumentNullException>());
        }
    }
}
